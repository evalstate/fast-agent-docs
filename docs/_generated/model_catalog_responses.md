| Catalog Entry | Resolves to | Status | Fast |
| --- | --- | --- | --- |
| `gpt-5.2` | `responses.gpt-5.2?reasoning=medium` | current | — |
| `gpt-5.3-chat-latest` | `responses.gpt-5.3-chat-latest?transport=auto` | current | — |
| `gpt-5.3-codex` | `responses.gpt-5.3-codex?reasoning=high` | current | — |
| `gpt-5.4` | `responses.gpt-5.4?reasoning=medium` | current | — |
| `gpt-5.4-mini` | `responses.gpt-5.4-mini?reasoning=medium` | current | yes |
| `gpt-5.4-nano` | `responses.gpt-5.4-nano?reasoning=medium` | current | yes |
